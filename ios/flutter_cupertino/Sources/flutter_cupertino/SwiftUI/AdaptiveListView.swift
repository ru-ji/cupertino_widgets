...
struct AdaptiveListView: View {
    ...
    /// Inset-grouped card corner radius. Overridable from Dart to match the
    /// running iOS version's Settings app (default 10).
    private var cornerRadius: CGFloat { CGFloat(config.cornerRadius ?? 10) }
